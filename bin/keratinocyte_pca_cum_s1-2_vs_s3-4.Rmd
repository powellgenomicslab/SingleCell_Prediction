...
subtitle: "Batch effect consideration"
...
```{r read_pca_data}
f.path <- "../2017-08-25_keratinocyte_pca_mds_dm/"
pca <- readRDS(paste0(f.path, "keratinocyte_pca.RDS"))
```
...
# expression.data <- readRDS("../data/25-08-2017_APC_E7_SCRNASEQ/keratinocyte_exp_matrix.RDS")
# expression.data<- expression.data[sample(x = seq_len(nrow(expression.data)), size = 300), 
#                                   sample(x = seq_len(ncol(expression.data)), size = 200)]
```
...
```{r set_clusters}
groups <- factor(ifelse(samples == 1 | samples == 2, "Batch_1", "Batch_2"))
positive.class <- "Batch_1"
negative.class <- "Batch_2"
```
