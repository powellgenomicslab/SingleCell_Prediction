...
output <- setOutput("2019-07-04", "scn_xin")


trainData <- readRDS("data/2018-04-15_pancreas_baron/baron-human.rds")
testData <- readRDS("data/2018-04-15_pancreas_xin/xin.rds")
...
level <- c("alpha", "beta", "delta", "gamma")
i <- which(true %in% c("alpha", "beta", "delta", "gamma"))
...
mapply(function(x,y) x/y, props, colSums(props)) %>% 
  ...
  round(2) %>% 
  .[,level] %>% 
  write.table(file = here(output, "xin_prop.txt"), sep = "\t", row.names = FALSE)  

props %>% 
  .[, level] %>% 
  write.table(file = here(output, "xin_counts.txt"), sep = "\t", row.names = FALSE)
