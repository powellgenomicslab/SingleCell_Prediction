...
```{r mds}
mds <- readRDS("../2017-08-21_hbca_mds/hipsc_mds.RDS")
...
mds %>%
  .[["points"]] %>% 
  as.data.frame() %>% 
  mutate(type = metadata[match(metadata$TYPE, row.names(.)), "group", drop = TRUE]) -> mds.format

# mds.format$type<- factor(ifelse(is.na(mds.format$type), "Other", as.character(mds.format$type)))
