...
output <- "results/2018-04-15_baron_processing"


expData <- counts(baron)
metadata <- colData(baron)
rm(baron)
...
saveRDS(cpm, 
        file = here(output, "baron_processed_cpm.RDS"))

saveRDS(newMetadata, 
        file = here(output, "baron_processed_metadata.RDS"))
