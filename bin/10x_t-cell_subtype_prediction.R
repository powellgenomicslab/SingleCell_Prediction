...
# title: T-cell subtype prediction
...
output_dir_name <- "pbmc_t-cell_prediction" # <------ Output directory
...
input %>% 
  ...
  str_subset(regex("^(?!(cd14_monocytes|cd34|b_cells|cd56_nk))")) -> filenames
...
data@meta.data$layer2.1 <- ifelse(grepl("cytotoxic", 
                                        data@meta.data$cellType), 
                                  "cytotoxic", 
                                  "non_cytotoxic") %>% as.factor()

p1 <- plotPCA(data, group = "layer2.1", 1,2)
p2 <- plotPCA(data, group = "layer2.1", 2,3)
p3 <- plotPCA(data, group = "layer2.1", 1,3)
...
plot_grid(p1  + theme(legend.position = "none"), 
          ...
          legend, rel_widths = c(1, 1, 1, 0.3),
          nrow = 1)

# Get feature space -------------------------------------------------------

sc_pred <- getFeatureSpace(data, pVar = "layer2.1")
...
combined@meta.data$layer2.1 <- ifelse(grepl("cytotoxic", 
                                        combined@meta.data$cellType), 
                                  "cytotoxic", 
                                  "non_cytotoxic") %>% as.factor()
...
crossTab(sc_pred, "layer2.1", "pred") %>% 
  knitr::kable() %>% 
  writeLines(here(output, "crosstab_prop.txt"))

crossTab(sc_pred, "layer2.1", "pred", prop = FALSE) %>% 
  knitr::kable() %>% 
