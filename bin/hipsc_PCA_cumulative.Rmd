...
predict.all <- function(dataset){
  TrainPredict(dataset$dataset.params, dataset$training.params, methods = 'rf', n.rep = 2, n.cores = 2)
}

res <- lapply(PC.datasets[1:2], predict.all)

```

```{r get_accuracy}
getAccuracy <- function(dataset.res){
dataset.res %>%
...
```{r get_accuracy_lasso}
accuracy <- lapply(res, getAccuracy) %>% 
  mapply(function(x, x.name) data.frame(Model = x$Model, Accuracy = x$Accuracy, n.pca = x.name), ., names(.), SIMPLIFY = FALSE) %>% 
  Reduce(rbind, .) %>% 
  filter(Model == "rf") %>% 
  select(-Model)
```
