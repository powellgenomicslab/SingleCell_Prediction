...
#for DE genes
i <- 1
results <- foreach(i = iter, .combine = comb, .multicombine = TRUE, .init = list(list(), list(), list(), list(), list(), list()), .packages = c("glmnet", "earth")) %dopar% {
  ...
  i <- i  + 1
  return(list(lasso, elastic.net.0.9, elastic.net.0.5, elastic.net.0.1, ridge, mars))
...
```{r sets_listnames}
names(results) <- c('lasso', 'elastic.net.0.9', 'elastic.net.0.5', 'elastic.net.0.1', 'ridge', 'mars')
```
