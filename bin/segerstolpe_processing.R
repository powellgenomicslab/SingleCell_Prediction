...
output <- "results/2018-04-15_segerstolpe_processing"


expData <- counts(segerstolpe)
metadata <- colData(segerstolpe)
rm(segerstolpe)
...
saveRDS(cpm, 
        file = here(output, "segerstolpe_processed_cpm.RDS"))

saveRDS(newMetadata, 
        file = here(output, "segerstolpe_processed_metadata.RDS"))
