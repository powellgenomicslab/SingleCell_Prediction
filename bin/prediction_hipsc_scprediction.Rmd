...
results <- foreach(i = iter, .combine = comb, .multicombine = TRUE, .init = list(list(), list(), list(), list(), list(), list(), list()), .packages = c("scPrediction")) %dopar% {
  ...
  # Run Naive Bayes
  suppressWarnings(predict.marker <- RunCaret(test,
                             training,
                             response,
                             method = 'nb',
                             seed = 92))
  
  naive.bayes <- ExtractResults(training = training, prediction = predict.marker,
                         clusterID.select = clusterID.select,
                         cell.class = cell.ids.cluster)
  ...
  svm.linear <- ExtractResults(training = training, prediction = predict.marker,
                         clusterID.select = clusterID.select,
                         cell.class = cell.ids.cluster)
  ...
  glm <- ExtractResults(training = training, prediction = predict.marker,
                         clusterID.select = clusterID.select,
                         cell.class = cell.ids.cluster)
  ...
  neural.network <- ExtractResults(training = training, prediction = predict.marker,
                         clusterID.select = clusterID.select,
                         cell.class = cell.ids.cluster)
