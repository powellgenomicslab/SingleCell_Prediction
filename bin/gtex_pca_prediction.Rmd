...
performance %>%
  ...
  summarise_at(.vars = c("Accuracy", "Sensitivity", "Specificity", "Kappa", "AUC"),
               .funs = funs(median)) %>%
  ungroup(n.ev)-> performance.median

metrics <- gather(data = performance.median, key = "Metric", value = "Value", 3:7)
...
metrics %>% 
  ...
  mutate(var.scale = ( (variance - min(variance)) / (max(variance) - min(variance)) ) * (max(Value) - min(Value)) + min(Value)) -> accuracy.scaled

p.accuracy <- ggplot(accuracy.scaled, aes(x = n.ev, y = Value)) +
  ...
  theme(axis.title.x = element_text(size = 19, face = "bold"),
        axis.title.y = element_text(size = 19, face = "bold"),
        strip.text.x = element_text(size = 17, face = "bold"),
        strip.background = element_rect(fill = "#fee6ce")) +
  scale_y_continuous(sec.axis = sec_axis(~( (. - min( accuracy.scaled$Value)) / (max( accuracy.scaled$Value) - min( accuracy.scaled$Value)) ) * (max( accuracy.scaled$variance) - min( accuracy.scaled$variance)) + min( accuracy.scaled$variance)))
```

```{r plot_accuracy}
metrics %>% 
  filter(Metric == "AUC", Model == "svmPoly") %>%
  mutate(var.scale = ( (variance - min(variance)) / (max(variance) - min(variance)) ) * (max(Value) - min(Value)) + min(Value)) -> auc.scaled

p.auc <-  ggplot(auc.scaled, aes(x = n.ev, y = Value)) +
  geom_point(color = "#de2d26") +
  geom_line(aes(x = as.numeric(n.ev)), color = "#de2d26") +
  geom_point(aes(x = as.numeric(n.ev), y = var.scale), color = "#636363", alpha = 0.5) +
  geom_line(aes(x = as.numeric(n.ev), y = var.scale), color = "#636363", alpha = 0.5) +
  ...
  theme(axis.title.x = element_text(size = 19, face = "bold"),
        ...
        strip.text.x = element_text(size = 17, face = "bold"),
        strip.background = element_rect(fill = "#fee6ce")) +
  scale_y_continuous(sec.axis = sec_axis(~( (. - min( auc.scaled$Value)) / (max( auc.scaled$Value) - min( auc.scaled$Value)) ) * (max( auc.scaled$variance) - min( auc.scaled$variance)) + min( auc.scaled$variance)))
```


```{r}
metrics %>% 
  filter(Metric == "Kappa", Model == "svmPoly") %>%
  mutate(var.scale = ( (variance - min(variance)) / (max(variance) - min(variance)) ) * (max(Value) - min(Value)) + min(Value)) -> kappa.scaled

p.kappa <- ggplot(kappa.scaled, aes(x = n.ev, y = Value)) +
  geom_point(color = "#de2d26") +
  geom_line(aes(x = as.numeric(n.ev)), color = "#de2d26") +
  geom_point(aes(x = as.numeric(n.ev), y = var.scale), color = "#636363", alpha = 0.5) +
  geom_line(aes(x = as.numeric(n.ev), y = var.scale), color = "#636363", alpha = 0.5) +
  ...
  theme(axis.title.x = element_text(size = 19, face = "bold"),
        ...
        strip.text.x = element_text(size = 17, face = "bold"),
        strip.background = element_rect(fill = "#fee6ce")) +
 scale_y_continuous(sec.axis = sec_axis(~( (. - min( kappa.scaled$Value)) / (max( kappa.scaled$Value) - min( kappa.scaled$Value)) ) * (max( kappa.scaled$variance) - min( kappa.scaled$variance)) + min( kappa.scaled$variance), name = "Variance explained (%)"))
```


```{r}
p <- plot_grid(p.accuracy, p.auc, p.kappa, labels = c("A", "B", "C"), nrow = 1)
```
