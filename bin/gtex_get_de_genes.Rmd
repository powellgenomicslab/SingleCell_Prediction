...
cont.matrix <- makeContrasts(cont = heart_left_ventricle - heart_atrial_appendage, 
                             levels = design)
...
```{r plot_DE_genes}
plotMD(fit.cont, coef = 1, status = summa.fit[, "cont"], main = paste(positive.class, "vs.", negative.class))
```
...
```{r save_results}
write.csv(significant.genes, file = paste0("../results/2017-09-21_gtex_de_genes/", positive.class, "_vs_", negative.class, ".csv"), row.names = FALSE)
```
