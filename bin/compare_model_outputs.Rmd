...
results.path <- file.path('..','results','2017-05-22_IPSCAnalysis', '2017-05-22_n100_replicates', 'All_List_cluster_1vs_RemainingClusters_n100-replicates.RDS')
...
ggplot(models, aes(x = model, y = accurate/(accurate + inaccurate), fill = model)) +
  ...
  ggtitle("Model accuracy comparison for HiPSCs", subtitle = paste(n.bootstrap, "bootstrap replicates were performed. Cluster 1 versus clusters 2,3 and 4")) +
  xlab("Model") +
  ylab("Accuracy") +
  theme_minimal() +
  scale_color_brewer()
...
ggplot(models.p.deviance.label, aes(x = deviance, y = 1)) +
  ...
  scale_x_continuous(limits = c(0, 1)) + 
  ggtitle('Classification Subpop 1 vs Subpop 2, 3 and 4', subtitle = 'Lasso') +
