...
predictor.training <- data.train.test$training
response <- data.train.test$response

# Fit model
fit <- glmnet(x = predictor.training, 
              y = response, 
              family = "binomial", 
              alpha = 0.5)
...
cvfit <- cv.glmnet(x = predictor.training, 
                   y =response, 
                   family = "binomial", 
                   type.measure = "class", 
                   alpha = 0.5)
...
cvfit.dev <- as.data.frame(as.matrix(print(cvfit$glmnet.fit)))
