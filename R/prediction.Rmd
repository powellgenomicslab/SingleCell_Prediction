...
```{r run_predictions}
#for DE genes
results <- foreach(i = iter, .combine = comb, .multicombine = TRUE, .init = list(list(), list()), .packages = "glmnet") %dopar% {
  ...
  prediction <- predict.marker$prediction
  elastic.net <- ExtractResults(training = training, prediction = prediction, cell.ids.cluster = cell.ids.cluster)

  return(list(lasso = lasso, elastic.net = elastic.net))
}
```


```{r sets_listnames}
names(results) <- c('lasso', 'elastic.net')
```
